...
describe('<FullHeader />', () => {
    ...
    context('bgColor', () => {
        it('should have background-color #ccc when none passed', () => {
            const wrapper = shallow(<FullHeader title="TDD" />);
            expect(wrapper).to.have.style('background-color').equal('#ccc');
        });

        it('should have background-color equal to #000 when passed', () => {
    ...
    context('textColor', () => {
        it('should have color #fff when none passed', () => {
            const wrapper = shallow(<FullHeader title="TDD" />);
            expect(wrapper).to.have.style('color').equal('#fff');
        });

        it('should have text-color equal to #000 when passed', () => {
            const wrapper = shallow(<FullHeader title="TDD" textColor="#000" />);
            expect(wrapper).to.have.style('color').equal('#000');
        });
    });

    context('font', () => {
        it('should have font equal sans-serif when none passed', () => {
            const wrapper = shallow(<FullHeader title="TDD" />);
    ...
    context('bgImg', () => {
        it('should have background-image equal to empty when none passed', () => {
            const wrapper = shallow(<FullHeader />);
